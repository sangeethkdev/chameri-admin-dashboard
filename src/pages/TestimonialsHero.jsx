...
const TestimonialsHero = () => {
  ...
  const heroMutation = useMutation({
    mutationFn: async () => {
      ...
      // anything over ~4.5MB.
      let mobileImageUrl = existingMobileImage;
      if (newMobileImage) {
        const uploadedMobile = await uploadToCloudinary(newMobileImage, { folder: "chameri/testimonials" });
        mobileImageUrl = uploadedMobile.url;
      }

      let imageUrl = existingImage;
      if (newImage) {
        const uploaded = await uploadToCloudinary(newImage, { folder: "chameri/testimonials" });
        imageUrl = uploaded.url;
      }

      return api.put("/testimonials-main/main/hero", {
        heading,
        image: imageUrl,
    ...
    onError: (err) =>
      toast.error(err.response?.data?.message || "Failed to save Testimonials Hero section"),
  });
  ...
  return (
    <div className="space-y-6 max-w-4xl pb-10">
      ...
      <div className="bg-dark-800 p-6 rounded-3xl mb-6 shadow-sm border border-surface-border">
        <h1 className="text-2xl font-bold text-white">Testimonials — Hero Section</h1>
        <p className="text-gray-400 text-sm mt-1">
          Manage the hero image and heading for the Testimonials page.
        </p>
      </div>
