...
const AboutFounder = () => {
  ...
  const founderFlash = useFlashSuccess();
  ...
  const founderMutation = useMutation({
    ...
    onError: (err) => toast.error(err.response?.data?.message || "Failed to save Founder section"),
  });
  ...
  return (
    <div className="space-y-6 max-w-4xl pb-10">
      <div className="bg-dark-800 p-6 rounded-3xl mb-6 shadow-sm border border-surface-border">
        <h1 className="text-2xl font-bold text-white">About Us — Founder Section</h1>
        <p className="text-gray-400 text-sm mt-1">Manage the founder content for the About page.</p>
      </div>

      <FormCard
        title="Founder Section"
        icon={ImageIcon}
        onSave={() => founderMutation.mutate()}
        isSaving={founderMutation.isLoading}
        saved={founderFlash.saved}
      >
