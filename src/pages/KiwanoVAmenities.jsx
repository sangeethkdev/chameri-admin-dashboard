...
const KiwanoVAmenities = () => {
  ...
  useEffect(() => {
    if (!data) return;
    setHeading(data?.amenitiesSection?.heading || "");
    setSubheading(data?.amenitiesSection?.subheading || "");
  }, [data]);

  const mutation = useMutation({
    mutationFn: async () => {
      return api.put("/kiwano-villament/main/amenities-section", { heading, subheading });
    },
    onSuccess: () => {
      flash.flash();
      qc.invalidateQueries(["kiwano-villament-main"]);
    },
    onError: (err) => toast.error(err.response?.data?.message || "Failed to save Amenities section"),
  });
  ...
  return (
    <div className="space-y-6 max-w-4xl pb-10">
      <div className="bg-dark-800 p-6 rounded-3xl mb-6 shadow-sm border border-surface-border">
        <h1 className="text-2xl font-bold text-white">Kiwano Villament — Amenities Section</h1>
        <p className="text-gray-400 text-sm mt-1">Manage the amenities section text content.</p>
      </div>
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <InputField label="Heading" value={heading} onChange={e => setHeading(e.target.value)} placeholder="e.g. World Class Amenities" />
          <InputField label="Subheading" value={subheading} onChange={e => setSubheading(e.target.value)} placeholder="e.g. Discover a lifestyle like no other" />
        </div>
      </FormCard>
    </div>
  );
...
export default KiwanoVAmenities;
